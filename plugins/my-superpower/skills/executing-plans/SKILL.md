...
- You have a plan from my-superpower:writing-plans and your human partner
...
- Tasks are mostly independent — the same precondition as
  my-superpower:subagent-driven-development.
...
Prefer my-superpower:subagent-driven-development when your human partner
...
The workspace and ledger are shared with my-superpower:subagent-driven-development
...
run `bash ../subagent-driven-development/scripts/task-brief-html --plan PLAN_FILE`
and read the file it writes. If the plan names a Spec, read that too: the spec is the
authority the plan argues from, and conflicts inside the plan resolve
against it. A plan with no reachable spec gets a ledger note saying so —
rulings made without one are provisional.

**REQUIRED SUB-SKILL:** load my-superpower:test-driven-development now,
...
- Run this skill's `scripts/task-start PLAN_FILE N`. It prints the brief
  ...
  **If PLAN_FILE ends in `.html`**, `task-start` cannot read it — run
  `bash ../subagent-driven-development/scripts/task-brief-html PLAN_FILE N && git rev-parse HEAD`
  instead: same brief path, same BASE, still one call. `task-done` works
  unchanged for either format.
...
order under my-superpower:test-driven-development, loaded at setup. A test
...
- **Matches.** Next step.
- **The code is wrong.** Use my-superpower:systematic-debugging. Find the
...
**REQUIRED SUB-SKILL:** my-superpower:verification-before-completion governs
...
[Read plan body once: bash ../subagent-driven-development/scripts/task-brief-html --plan docs/mySuperpower/plans/feature-plan.html; spec read]
[Resolve workspace: sdd-workspace docs/mySuperpower/plans/feature-plan.html — no ledger inside, fresh start]
[Pre-flight scan: 2 shared-interface rows, 4 self-consistency rows, clean; written to ledger]
[Create todos for all tasks]

Task 1: Hook installation script

[task-brief-html plan 1 + rev-parse → brief read; BASE a1b2c3d]
...
[task-brief-html plan 2 + rev-parse → brief read; BASE d4e5f6a]
...
Using my-superpower:finishing-a-development-branch.
```
