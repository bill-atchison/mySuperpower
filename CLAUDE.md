...
The git-tracked source stays **pristine**. Every mySuperpower transform — the HTML
overlays, the `my-superpower:` namespace cascade, the `docs/mySuperpower/` output paths,
and the manifest identity — happens at BUILD time in `scripts/build-mysuperpower.ps1`, so
upstream skill edits merge cleanly.

```
git branch backup/pre-<new-upstream-version>-sync        # safety net
git fetch upstream --tags
git merge upstream/main                                  # conflicts are the version field
                                                         # in the declared manifests:
                                                         # resolve those to upstream
scripts/bump-version.sh --sync <new-upstream-version>    # e.g. 6.3.0 -> 6.3.0+fork.1
# add the CHANGELOG entry, then build — this is THE anchor test:
pwsh -NoProfile -File scripts/build-mysuperpower.ps1
scripts/bump-version.sh --check
git add -u && git commit
```

The build fails loud when a required overlay anchor is missing, which is how a reworded
upstream skill is caught — fix the `find` string in `overlays/<skill>/overlay.json` and
rebuild.

Do NOT run `scripts/apply-my-superpower-brand.*`. It is a superseded source-branding helper
from the pre-overlay architecture; the source manifests intentionally keep upstream’s
`name: "superpowers"`.

**Prerequisites:** `jq` and `yq` (bump-version.sh reads the Hermes YAML manifest), plus
PowerShell 7 (`pwsh`) for the build and publish scripts.
